...
import { downloadContractFile } from '../services/api'
...
function Cell({ children, className = '' }) {
  return (
    <span className={`text-[12.5px] text-[#475569] break-words whitespace-normal leading-snug block ${className}`}>
      {children ?? '—'}
    </span>
  )
}
  ...
  return (
    <>
      <button ref={btnRef} onClick={handleOpen}
        ...
        {status}
        <svg className="w-[11px] h-[11px] opacity-50 ml-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
      ...
      {open && (
        ...
        >
          <p className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-widest px-3.5 pb-1.5">Status dəyiş</p>
          {others.map(opt => {
...
function SignatureCell({ url }) {
  const [failed, setFailed] = useState(false)
  useEffect(() => { setFailed(false) }, [url])

  return (
    <div className="h-10 min-w-[88px] border border-[#e2e8f0] rounded-lg bg-white flex items-center justify-center shadow-sm px-2">
      {url && !failed ? (
        <img
          src={url}
          alt="İmza"
        ...
        <span className="text-[11px] text-[#cbd5e1] text-center px-1 leading-tight">
          {url ? 'İmza yüklənmədi' : '—'}
        </span>
      )}
    </div>
          ...
          <div className="relative" ref={filterRef}>
            ...
            {filterOpen && (
              <div className="absolute right-0 top-full mt-2 w-[196px] bg-white border border-[#e2e8f0]
                ...
                <p className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-widest px-3 pb-2">Statusa görə</p>
                ...
                <div className="px-3 pt-2">
                  <button onClick={() => { onFilterChange(pendingFilter); setFilterOpen(false) }}
                    className="w-full py-[9px] bg-[#001A5C] text-white text-[13px] font-semibold rounded-[8px] hover:bg-[#002580] transition-colors">
                    Tətbiq et
                  </button>
                </div>
              </div>
        ...
        <table className="w-full" style={{ borderCollapse: 'collapse', minWidth: '1100px' }}>
          <thead>
            <tr style={{ background: 'linear-gradient(180deg,#f8fafc 0%,#f1f5f9 100%)' }}
              ...
              {HEADERS.map((h, i) => (
                <th key={i} className="text-left px-4 py-3 text-[10px] font-bold text-[#94a3b8] uppercase tracking-[0.08em] whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              const displayName = row.fullName && row.fullName !== 'Bilinməyən'
                ? row.fullName
                : [row.firstName, row.lastName].filter(Boolean).join(' ') || '—'

              return (
                <tr key={row.id} className="border-b border-[#f8fafc] hover:bg-[#fafbff] transition-colors align-top">

                  {/* ID + contractNumber (API sırası) */}
                  <td className="px-4 py-4 min-w-[130px]">
                    <div className="flex flex-col gap-1.5">
                      <span className="inline-flex self-start items-center px-2 py-0.5 rounded-md
                        text-[11px] font-bold font-mono text-[#001A5C] bg-[#f0f4ff] border border-[#c7d7fe]">
                        #{row.id}
                      </span>
                      <span className="text-[12px] font-semibold text-[#3b82f6] font-mono break-words whitespace-normal leading-snug">
                        {row.contractNumber || '—'}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4 min-w-[150px]">
                    <div className="flex items-start gap-2.5">
                      <div
                        ...
                        style={{ background: `hsl(${(row.id * 53) % 360},58%,48%)` }}
                      >
                        {initial}
                      </div>
                      <span className="text-[13px] font-semibold text-[#0f172a] break-words whitespace-normal leading-snug pt-0.5">
                        {displayName}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4 min-w-[90px]">
                    <Cell className="text-[#64748b]">{row.fatherName}</Cell>
                  </td>

                  <td className="px-4 py-4 min-w-[95px]">
                    <span className="inline-block text-[11.5px] font-mono text-[#475569] bg-[#f8fafc] px-2 py-1
                      rounded-md border border-[#e8edf5] tracking-wide break-all whitespace-normal">
                      {row.fin || '—'}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <SignatureCell url={row.signatureUrl} />
                  </td>

                  {/* Müqavilə Adı — PDF + nömrə (skrinşot kimi) */}
                  <td className="px-4 py-4 min-w-[130px]">
                    <button
                      type="button"
                      className="flex items-start gap-2 text-left group/pdf w-full"
                      onClick={async () => {
                        try {
                          await downloadContractFile(row.id, row.contractNumber)
                          toast.success('Müqavilə yükləndi', { duration: 2000 })
                        } catch (err) {
                          toast.error(err.message || 'Müqavilə yüklənə bilmədi', { duration: 3000 })
                        }
                      }}
                    >
                      <PdfIcon />
                      <span className="text-[12px] text-[#3b82f6] font-medium group-hover/pdf:underline break-words whitespace-normal leading-snug">
                        {row.contractNumber || 'Müqavilə sənədi'}
                      </span>
                    </button>
                  </td>
                  ...
                  <td className="px-2 py-4 text-center">
                    <button onClick={e => handleMenuOpen(e, row)}
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-[#94a3b8]
                        hover:bg-[#e8edf5] hover:text-[#475569] transition-all">
      ...
      {menuCtx && (
        ...
        >
          <div className="px-3.5 py-1.5 border-b border-[#f1f5f9] mb-1">
            <p className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-wider break-words">{menuCtx.name}</p>
          </div>
